...
1. **The vouch carries a date** — `[VOUCHED @hyf0 2026-07-08]`. It is metadata on the existing bit, not a new kind of stamp: it changes nothing about what a vouch *means*, only records when it was made, so "vouched but stale" becomes a state you can see instead of one you discover the hard way. Agents date every stamp they add: most stamps are typed by an agent on a human's instruction, and the agent knows the date, so it costs nothing. A hand-typed stamp may skip it — git can recover the date later, but the reader shouldn't need archaeology.
