...
1. **The vouch carries a date** — `[VOUCHED @hyf0 2026-07-08]`. It is metadata on the existing bit, not a new kind of stamp: it changes nothing about what a vouch *means*, only records when it was made, so "vouched but stale" becomes a state you can see. The normative rule (agents date every stamp they add; a hand-typed stamp may skip it) lives in [the README's stamp section](../../README.md#the-mechanism-the-provenance-stamp).
