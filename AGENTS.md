...
- **The adoption block** includes its `PCR:START` / `PCR:END` markers and lives in `README.md`, `README.zh-CN.md`, `skills/pcr-setup/SKILL.md`, and above in this file — all four byte-identical. Editing the block means editing all four by hand; once the change lands on `main`, updating the installed skill and re-running `/pcr-setup` here refreshes this file's copy, so every merged block change is also a live test of the update path — until then the installed skill still carries the old block, and re-running it would revert this copy.
