...
"Record anything worth keeping" gives a new adopter no first move, and an empty records folder earns nothing on day one — exactly when an adopter decides whether the practice is worth keeping. So the block fixes one list, **the basics**: `goal.md` (what this is trying to be, for whom, and the non-goals), `technology-stack.md`, `architecture.md`, `conventions.md`, `gotchas.md`, and — for a project with a visual surface — a root `DESIGN.md` in the [design.md](https://github.com/google-labs-code/design.md) format. Each holds what nothing else can say — a goal record keeps the intent and refusals a user-facing README won't carry; a stack record keeps the whys the manifest can't — and the per-topic reasoning lives in [starter-set](./.agents/docs/starter-set.md).
