...
- **Provenance.** Unstamped text is AI-accumulated: challenge and verify it freely. `[VOUCHED @handle YYYY-MM-DD]` means the named human explicitly accepted the covered words as current project direction.
  ...
  - Materially editing covered words removes the stamp until the human re-vouches. Heading-level changes, inserted or removed boundary headings, and moved stamps are material whenever they change which words are covered; a formatting-only change keeps the stamp only if the covered words stay identical.
