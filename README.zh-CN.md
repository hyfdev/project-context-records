...
- **`[VOUCHED @handle 2026-07-08]` = 有人为它背书，日期即背书之时。** 视为地基：不要为验证而验证、为重开而重开——只在新证据、约束变化、或人发话时才做。章可以标在单独一行上（行尾），或整个文件上（文件顶部），它代表明确的人类背书：由人亲手输入，或由 agent 仅在人的明确指示下加上——agent 落章必带日期，人手敲的可以省略。人读过一行而没有反对，永远不算背书。
...
**日期让过期变得可见。** 它仍是同一个比特，只是加了时间戳——不是一种新章——它把"背书过但已过期"变成看得见的状态：[提炼流程](#工作流程积累--提炼)可以机械地标出比其覆盖的代码更老的背书，而不靠主观判断（[freshness](./.agents/docs/freshness.md)）。
