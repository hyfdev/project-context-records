...
大多数仓库已经握有这一层的碎片——根目录的 `ARCHITECTURE.md` 或 `DESIGN.md`、一个 `adr/` 文件夹、内部文档。它们在所有要紧的意义上都是 records：**登记（enroll）它们**——map 里一行、指向它住的地方、遵守同样的规则——而不是搬动它们，或起草会渐行渐远的孪生副本（[existing-docs](./.agents/docs/existing-docs.md)）。
...
"记下任何值得保留的东西"没有给新采纳者第一步棋，而空的 records 文件夹在第一天毫无回报——恰恰是采纳者决定这套实践值不值得坚持的时刻。所以 block 固定了唯一一份清单，**the basics**：`goal.md`（它要成为什么、为谁、以及 non-goals）、`technology-stack.md`、`architecture.md`、`conventions.md`、`gotchas.md`，以及——面向有视觉表面的项目——按 [design.md](https://github.com/google-labs-code/design.md) 格式放在根目录的 `DESIGN.md`。每一条都靠装下别处装不了的东西挣得位置——goal 记录装着面向用户的 README 不会写的意图与拒绝；stack 记录装着 manifest 说不出的 why——逐条的理由在 [starter-set](./.agents/docs/starter-set.md)。
