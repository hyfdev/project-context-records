...
- [ ] 5. If the records folder already has records, reconcile old starter names: a `goal.md` from an earlier PCR version is today's `intent.md` — offer to rename it, updating map routes and links in the same change. If both files exist, surface the overlap instead of merging silently. Other legacy records (a `conventions.md`, archived run files) need nothing: they stay valid free-form records. Records are never otherwise touched.
- [ ] 6. If the records folder has no records yet, offer to seed:
  ...
  - **Existing codebase**: the seed is the **suggested topics**, drafted from what the repo itself shows (README, manifests, config, git history). First check for existing docs that already cover a topic — `DESIGN.md`, `ARCHITECTURE.md`, `docs/`, an `adr/` folder: **enroll** those instead of duplicating them, with a map route pointing at each where it lives (create the map if needed — records outside the folder are exactly what a map is for). Leave machine-checkable constraints to the repository's own types, tests, lints, or CI — seeding drafts records, never enforcement — and keep only their rationale and reconsideration conditions in prose. Back factual claims with durable evidence, never a temporary path or unpreserved screenshot. Then draft only the gaps — only what the repo gives evidence for, skipping the rest — and leave everything unstamped: it is AI-accumulated until a human reviews and vouches it. The value in each is what stronger artifacts and the code alone *cannot* say — not a restatement of them:
    ...
    - `DESIGN.md` — only if the project has a visual surface (a UI, a site, themed output): its visual identity for coding agents — design tokens plus their why — following the design.md spec (https://github.com/google-labs-code/design.md). Draft it in the records folder by default (the design.md spec fixes no location) and enroll it in the map. Whenever a `DESIGN.md` exists — found or drafted — suggest wiring its linter into the project's own checks (CI or pre-commit) with the file's actual path rather than running it once (e.g. `npx @google/design.md lint .agents/docs/DESIGN.md`); the spec documents a dot-free `designmd` form for platforms where the direct command misfires; for a project with no visual surface, don't mention the file or the linter at all.
- [ ] 7. Tell the user in one or two lines what changed, including any legacy project rules moved outside the markers. Say that records live under the folder and a human can explicitly vouch a line, section, or whole file with `[VOUCHED @<their-handle> YYYY-MM-DD]`. If records were seeded, say they are unstamped drafts awaiting review.

## Opening a decision ledger (on the user's declaration)

A decision ledger is opt-in and per-area: it exists only after the human declares that an area records decisions ("design decisions get recorded from now on"). If the project does not carry the PCR block yet, stop and confirm adoption first (steps 1–4) instead of scaffolding an orphan ledger. When the declaration lands: if `<area>-decisions.md` does not exist, create it from the template below — next to the area's derived document (`DESIGN-decisions.md` beside `DESIGN.md`, typically both in the records folder), otherwise in the records folder. If it already exists (a legacy ledger predating this format), never overwrite or restructure it — keep its content and offer a migration to the template shape for the human to approve. Either way, ensure the map routes to it exactly once. You may suggest a ledger when an area's judgments grow dense; only the human's declaration creates one.
